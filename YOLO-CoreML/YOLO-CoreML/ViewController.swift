...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let doubleTabGesture = UITapGestureRecognizer(target: self, action: #selector(handleDoubleTap))
        doubleTabGesture.numberOfTapsRequired = 2
        videoPreview.addGestureRecognizer(doubleTabGesture)
    }
    
    @objc func handleDoubleTap() {
        self.videoCapture.stop()
        if (self.yoloVersion == 2) {
            self.yoloVersion = 3
        } else {
            self.yoloVersion = 2
    ...
    func setUpVision() {
        guard let visionModel = try? VNCoreMLModel(for: self.yoloVersion == 2 ? TinyYOLO().model : Yolov3().model) else {
    ...
    func visionRequestDidComplete(request: VNRequest, error: Error?) {
        ...
        if self.yoloVersion == 2 {
            if let observations = request.results as? [VNCoreMLFeatureValueObservation],
                ...
                var boundingBoxes : [Prediction] = []
                boundingBoxes = yolo.computeBoundingBoxes(features: features)
        ...
        if self.yoloVersion == 3 {
            if let observations = request.results as? [VNCoreMLFeatureValueObservation] {
                var boundingBoxes : [Prediction] = []
                for feature in observations {
                    if feature.featureName == "Identity" {
                        boundingBoxes += yolov3.computeBoundingBoxes(features: feature.featureValue.multiArrayValue!, scale: 13, scaleIndex: 3)
                    } else if feature.featureName == "Identity_1" {
                        boundingBoxes += yolov3.computeBoundingBoxes(features: feature.featureValue.multiArrayValue!, scale: 26, scaleIndex: 2)
                    } else if feature.featureName == "Identity_2" {
                        boundingBoxes += yolov3.computeBoundingBoxes(features: feature.featureValue.multiArrayValue!, scale: 52, scaleIndex: 1)
                    }
                }
                // We already filtered out any bounding boxes that have very low scores,
                // but there still may be boxes that overlap too much with others. We'll
                // use "non-maximum suppression" to prune those duplicate bounding boxes.
                boundingBoxes = nonMaxSuppression(boxes: boundingBoxes, limit: YOLOv3.maxBoundingBoxes, threshold:  0.5)
                let elapsed = CACurrentMediaTime() - startTimes.remove(at: 0)
                showOnMainThread(boundingBoxes, elapsed)
            } else {
                print("BOGUS!")
    ...
    func showOnMainThread(_ boundingBoxes: [Prediction], _ elapsed: CFTimeInterval) {
        if drawBoundingBoxes {
            DispatchQueue.main.async {
                ...
                self.timeLabel.text = String(format: "YOLO v%d - Elapsed %.5f seconds - %.2f FPS", self.yoloVersion, elapsed, fps)
            }
        }
    }
    ...
    func show(predictions: [Prediction]) {
        for i in 0..<boundingBoxes.count {
            if i < predictions.count {
                ...
                let classLabel = self.yoloVersion == 2 ? yolov2_labels[prediction.classIndex] : yolov3_labels[prediction.classIndex]
