...
class YOLOv3 {
    ...
    public func computeBoundingBoxes(features: MLMultiArray, scale: Int, scaleIndex: Int) -> [Prediction] {
        if (features.count != 255*scale*scale) { return [] }
        ...
        for cy in 0..<gridHeight {
            for cx in 0..<gridWidth {
                for b in 0..<boxesPerCell {
                    ...
                    let w = exp(tw) * anchors_v3[scaleIndex - 1][2*b    ] // * blockSize
                    let h = exp(th) * anchors_v3[scaleIndex - 1][2*b + 1] // * blockSize
